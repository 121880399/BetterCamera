...
public final class CameraHelper {
    ...
    public static int calDisplayOrientation(Context context, @CameraConstant.Face int cameraFace, int cameraOrientation) {
        int displayRotation;
        ...
        if (cameraFace == CameraConstant.FACE_FRONT) {
    ...
    public static CamcorderProfile getCamcorderProfile(int currentCameraId, long maximumFileSize, int minimumDurationInSeconds) {
        if (maximumFileSize <= 0){
            return CamcorderProfile.get(currentCameraId, MediaConstant.QUALITY_HIGHEST);}

        int[] qualities = new int[]{MediaConstant.QUALITY_HIGHEST,
                MediaConstant.QUALITY_HIGH, MediaConstant.QUALITY_MEDIUM,
                MediaConstant.QUALITY_LOW, MediaConstant.QUALITY_LOWEST};
        ...
        return CameraHelper.getCamcorderProfile(MediaConstant.QUALITY_LOWEST, currentCameraId);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static CamcorderProfile getCamcorderProfile(@MediaConstant.Quality int mediaQuality, String cameraId) {
        if (TextUtils.isEmpty(cameraId)) {
            return null;
        }
        int cameraIdInt = Integer.parseInt(cameraId);
    ...
    public static CamcorderProfile getCamcorderProfile(@MediaConstant.Quality int mediaQuality, int cameraId) {
        if (mediaQuality == MediaConstant.QUALITY_HIGHEST) {
            return CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_HIGH);
        } else if (mediaQuality == MediaConstant.QUALITY_HIGH) {
        ...
        } else if (mediaQuality == MediaConstant.QUALITY_MEDIUM) {
        ...
        } else if (mediaQuality == MediaConstant.QUALITY_LOW) {
            if (CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_480P)) {
                return CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_480P);
            } else {
                return CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_LOW);
            }
        } else if (mediaQuality == MediaConstant.QUALITY_LOWEST) {
            return CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_LOW);
        } else {
            return CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_HIGH);
        }
